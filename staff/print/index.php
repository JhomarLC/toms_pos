...
class PDF extends FPDF
{
    ...
    function Header()
    {
        $this->Image('toms_logo.png', 25, 5, 30);
        $this->Ln(10);
    }
...
$order_date = $_POST['order_date']; // Use the actual order date here
...
$query = "SELECT o.*, a.full_name FROM orders as o INNER JOIN accounts as a ON o.account_id = a.account_id WHERE order_id = ?";
...
$pdf->Ln(5);
$pdf->Cell(0, 5, 'Cashier: ' . $order['full_name'], 0, 1, 'L');
$pdf->Cell(0, 5, '-----------------------------------------------------', 0, 1, 'L');
$pdf->Cell(0, 5, $order['order_method'], 0, 1, 'L');
$pdf->Cell(0, 5, '['. $order['payment_method'] .']', 0, 1, 'L');
$pdf->Cell(0, 5, '-----------------------------------------------------', 0, 1, 'L');
$stmt->close();
// QUERY
...
// Receipt content
$pdf->SetFont('Helvetica', '', 11);
$pdf->Cell(50, 5, 'Subtotal', 0);
$pdf->Cell(20, 5, 'P' . number_format(($order['total'] + $order['discount']) - $order['leftover'], 2), 0, 1, 'R');
$pdf->Cell(50, 5, 'Leftover', 0);
$pdf->Cell(20, 5, '- P' . number_format($order['leftover'], 2), 0, 1, 'R');
$pdf->Cell(50, 5, 'Discount', 0);
$pdf->Cell(20, 5, '- P' . number_format($order['discount'], 2), 0, 1, 'R');
$pdf->Ln(1);
